...
<div class="max-w-2xl mx-auto">
    <div style="margin-bottom: 50px;">
        <h1 class="kaira-section-heading" style="font-family: 'Marcellus', serif; font-size: 42px; color: #212529; margin-bottom: 10px; letter-spacing: 1px;">Edit Instansi</h1>
        <p style="font-family: 'Jost', sans-serif; color: #8f8f8f; font-size: 16px;">Edit informasi instansi</p>
    </div>
            ...
            <div style="margin-bottom: 30px;">
                <label style="display: flex; align-items: center; cursor: pointer;">
                    <input 
                        ...
                        value="1"
                        {{ old('status_checkbox', $instansi->status === 'active') ? 'checked' : '' }}
                ...
                <input type="hidden" name="status" value="{{ old('status', $instansi->status) }}" id="status-input">
                @error('status')
                    <p style="font-family: 'Jost', sans-serif; color: #dc3545; font-size: 12px; margin-top: 8px; display: flex; align-items: center; gap: 5px;">
                        <svg style="width: 16px; height: 16px;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Initialize value based on checkbox state
        if (checkbox.checked) {
            statusInput.value = 'active';
        } else {
            statusInput.value = 'suspended';
